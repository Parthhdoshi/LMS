...
const CourseContent = ({ id,user, progressDetails }: Props) => {
  const { data: contentData, isLoading,refetch } = useGetCourseContentQuery(id,{refetchOnMountOrArgChange:true});
  const [open, setOpen] = useState(false);
  const [route, setRoute] = useState('Login')
  const data = contentData?.content;
  const existingQuizResult = contentData?.quizResults
  const assignedQuiz = progressDetails?.assignedQuiz
  const taskData = contentData?.task
