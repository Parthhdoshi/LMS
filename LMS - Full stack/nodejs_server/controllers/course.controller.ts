  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (thumbnail) {
        ...
        const buffer = await Buffer.from(thumbnail.split(",")[1], "base64");

        const params: any = {
          Bucket: process.env.S3_BUCKET_NAME,
          Key: `${Date.now()}_course_thumbnail_${data.name}`,
          Body: buffer,
          ContentEncoding: "base64",
          ContentType: data?.thumbnail?.split(";")[0]?.split("data:")[1] || "",
        };
